<?php
/*
Plugin Name: Nexus Master
Version: 1
Description: Connects to LinedIn and performs some arbitrary unspecified function.
Author: Mikel King
...
// setsup the oauth tokens are read from service.php which simulates reading in from a db or some other source.

	if( isset($srvc_oauth_token) && isset($srvc_oauth_token_secret) ) {
		$oauth->setToken($srvc_oauth_token, $srvc_oauth_token_secret);
	} else {
		print_line("We had a service.dat file, but it didn't contain a token/secret?", null, STDERR ,__LINE__);
		//print_line("We had a service.dat file, but it didn't contain a token/secret?", null, __LINE__);
	}
